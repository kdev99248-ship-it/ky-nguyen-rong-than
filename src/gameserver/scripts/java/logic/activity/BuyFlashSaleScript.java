...
import Message.C2SActivityMsg.DoFlashSale;
import Message.C2SActivityMsg.DoFlashSaleID;
...
/**
 * 
 * @ClassName: BuyFlashSaleScript
 * @Description: 兑换限时兑换
 * @author tzyx
 * @date 2018年7月19日 下午1:10:04
 */
public class BuyFlashSaleScript implements IScript {
	private static Logger LOGGER = Logger.getLogger(BuyFlashSaleScript.class);
	...
	public Object call(String scriptName, Object arg) {
		...
		DoFlashSale req = (DoFlashSale) script.get(Key.ARG1);
		buyFalshSale(player, req);
		return null;
	}

	private void buyFalshSale(Player player, DoFlashSale req) {
		int num = req.getNum();
		if (num < 0) {
		...
		FlashSaleConfig config = map.get(req.getActivityId());
		if (null == config) {
		...
		if (config.getSaleTime() < System.currentTimeMillis()) {
			LOGGER.error("当前不在活动时间内");
		...
		for (FlashSaleItem item : config.getItems()) {
			if (item.getId().equals(req.getId())) {
				...
				use.addAll(BeanFactory.createProps(item.getExchanges().getId(), item.getExchanges().getNum() * num));
				StringBuilder sb1 = new StringBuilder();
				for (Item it : item.getAwards()) {
					sb1.append(it.getId()).append("_").append(it.getNum()).append(",");
				...
				player.getBackpackManager().addItems(reward, true, false, Reason.ACTIVITY_FLASH_SALE, "");
				// 增加次数
				item.setAllUsedTimes(item.getAllUsedTimes() + num);
				item.getUsedTimes().put(player.getPlayerId(), personNum + num);
				service.flashSaleNotify(player);
				...
				LogService.getInstance().logPlayerAction(player, DoFlashSaleID.DoFlashSaleMsgID_VALUE, config.getId(),
						sb.substring(0, sb.length() - 1), item.getId(), sb1.substring(0, sb1.length() - 1));
				break;
			}
		}
	}
