...
import Message.C2SRuneMsg.HeroPutOnRuneReq;
import Message.C2SRuneMsg.HeroPutOnRuneReqID;
 ...
 *
 */
public class ReqHeroPutOnRuneScript implements IScript {

	private final Logger logger = Logger.getLogger(ReqHeroPutOnRuneScript.class);
	...
	public Object call(String scriptName, Object arg) {
		...
		HeroPutOnRuneReq req = (HeroPutOnRuneReq) script.get(ScriptArgs.Key.ARG1);
		Player player = (Player) script.get(ScriptArgs.Key.PLAYER);
		putOnRune(req, player);
		return null;
	}

	private void putOnRune(HeroPutOnRuneReq req, Player player) {
		// 检查系统开放
		...
		Map<Integer, Rune> putOnRunes = new HashMap<>();
		Long runeId;
		Rune rune;
		t_runeBean runeBean;
		...
		for (int i = 0; i < runeIdsList.size(); i++) {
			...
			if (null != runeId) {
				...
				if (null != rune) {
					...
					if (!putOnRunes.containsKey(runeBean.getSeat())) {
						putOnRunes.put(runeBean.getSeat(), rune);
					} else {
						logger.info("多个符文同一位置");
						MessageUtils.sendPrompt(player, PromptType.ERROR, "多个符文同一位置");
						return;
					}
				} else {
		...
		for (Integer seat : putOnRunes.keySet()) {
			rune = putOnRunes.get(seat);
			// 判断位置是否已经有符文,有符文就拆下符文
			if (heroRuneInfo.getRuneId()[seat - 1] != 0) {
				runeManager.putDownRune(hero, seat, propMap, changeIds);
			}
			// 装上符文
			runeManager.putOnRune(hero, seat, rune.getId(), propMap, changeIds);
		}
		// 计算英雄符文套装
		...
		// 记录动作    英雄id,符文id列表
		LogService.getInstance().logPlayerAction(player, HeroPutOnRuneReqID.HeroPutOnRuneReqMsgID_VALUE,
